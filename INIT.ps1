...
<#
.SYNOPSIS
	Interactive initializer for the React SPA template.
...
.DESCRIPTION
	...
	(package.json, env/appsettings.json, README.md, public/site.webmanifest,
	specs/project.md), then formats the result.
...
Write-Banner 'REACT · INICIALIZAR PROYECTO' info
...
$basePathRaw = Read-Field 'Ruta base' -Required $false -Description 'Ruta pública (ej: siniestro). Vacío = raíz /'
...
$tokenFiles = @('package.json', 'env/appsettings.json', 'public/site.webmanifest', 'specs/project.md')
...
$startUrl = $basePath ? "http://localhost:3003/$basePath" : 'http://localhost:3003/'
...
Write-Next "pnpm start:dev   ·   $startUrl"
Write-Host ''
