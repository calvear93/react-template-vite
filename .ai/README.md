...
Tool-neutral home for the **spec-driven development (SDD)** workflow and the reusable
best-practice skills. This is the single source of truth for the skill procedures; each AI
tool gets a thin adapter that points here (same philosophy as [`AGENTS.md`](../AGENTS.md)).
...
.ai/
  ...
  prompts/         canonical task-prompt bodies (component/page/hook/test creation,
                   blueprints, reviews, llms.txt … — edit these)
  agents/          canonical agent definitions (blueprint, debugger, mentor, react)
...
`implement` step reuses the canonical task prompts in `.ai/prompts/` (component / page /
hook / test creation) instead of redefining them.
...
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GitHub Copilot | Native thin pointers under `.github/`: `prompts/spec-*.prompt.md` (SDD), `prompts/*.prompt.md` (tasks), `agents/*.agent.md`. Invoked as `/spec-specify`, `/component-creation`, … |
