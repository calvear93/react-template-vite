...
4. **Spot-check best practices:** components declarative + logic in hooks; config injected
   (no hardcoded values / scattered `import.meta.env`); accessible markup; no `any` left where
   a real type fits.
