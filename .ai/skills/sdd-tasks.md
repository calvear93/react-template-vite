...
3. **Make each task atomic and verifiable:**
    - One coherent unit (a schema, a hook, a component, a route, wiring) — small enough to
      implement and test in isolation.
    - **Test-first:** each task names the `*.spec.tsx`/`*.spec.ts` it adds/updates and the
      acceptance criterion it covers.
    - Explicit "done when" (an assertion or observable outcome).
4. **Order by dependency:** schemas/types → services + IoC bindings → hooks → components →
